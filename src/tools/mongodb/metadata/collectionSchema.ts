...
export class CollectionSchemaTool extends MongoDBToolBase {
    ...
    protected async execute({ database, collection }: ToolArgs<typeof this.argsShape>): Promise<CallToolResult> {
        ...
        const documents = await provider.find(effectiveDatabase, collection, {}, { limit: 5 }).toArray();
        ...
        if (fieldsCount === 0) {
            return {
                content: [
                    {
                        text: `Could not deduce the schema for "${effectiveDatabase}.${collection}". This may be because it doesn't exist or is empty.`,
                        type: "text",
                    },
                ],
            };
        ...
        return {
            content: [
                {
                    text: `Found ${fieldsCount} fields in the schema for "${effectiveDatabase}.${collection}"`,
                    type: "text",
                },
                {
